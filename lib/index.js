...
internals.Dispenser.prototype._onLineEnd = function () {
    ...
    if (this._state === internals.state.header) {
        ...
        if (disposition && disposition.filename !== undefined) {
        ...
        else if (this._headers['content-type'] == "application/octet-stream"){
            this._isPartBytes = true;
            this._partLength = NaN;
            if (this._headers['content-length']) {
                this._partLength = Number.parseInt(this._headers['content-length']);
            }

            if (Number.isNaN(this._partLength) && this._headers['content-range']) {
                const range = contentRange.format(this._headers['content-range']);
                this._partLength = range.first - range.limit - 1;
            }
        }
